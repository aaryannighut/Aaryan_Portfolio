...
export default function Navbar() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/35 backdrop-blur-md border-b border-white/5 py-2.5 md:py-4.5 px-6 md:px-12 lg:px-16">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-x-0 top-[56px] md:top-[72px] bottom-0 bg-[#0A0A0A]/98 backdrop-blur-lg z-40 border-t border-white/5 flex flex-col items-center justify-start py-10 px-6 md:px-12 md:py-16 overflow-y-auto"
          >
            <nav className="flex flex-col items-center justify-start gap-y-7 text-sm font-mono uppercase tracking-[0.25em]">
              {navItems.map((item) => (
                <motion.button
                  key={item.id}
                ...
                >
                  <item.icon className="w-5 h-5 text-gray-400 group-hover:text-[#FFD700] transition-colors duration-300" />
